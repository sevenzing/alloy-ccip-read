...
def main() -> None:
    ...
    new_text, count = re.subn(
        r'^alloy-ccip-read = "[^"]+"$',
        f'alloy-ccip-read = "{semver_range}"',
        readme_text,
        count=1,
        flags=re.MULTILINE,
    )
    if count == 0:
        print('Could not find alloy-ccip-read = "..." line in README.md', file=sys.stderr)
        sys.exit(1)

    README.write_text(new_text, encoding="utf-8")
    print(f'README.md: alloy-ccip-read = "{semver_range}" (from Cargo.toml {version})')
